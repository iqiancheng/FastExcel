...
public class TestReadAll extends TestCase {
	...
	private void basicDumpAll(File dir,int ssttype) throws ExcelException{
		File ff[]=dir.listFiles();
		for(int i=0;i<ff.length;i++){
			if(ff[i].getName().endsWith(".xls")){
				basicDump(ff[i], ssttype);
			}
		}
	}
	private void eventDumpAll(File dir,int ssttype) throws ExcelException{
		File ff[]=dir.listFiles();
		for(int i=0;i<ff.length;i++){
			if(ff[i].getName().endsWith(".xls")){
				eventDump(ff[i], ssttype);
			}
		}
	}
...
//				System.out.println(row + "," + col + "," + content);
	...
	public void testBasicDump1() throws ExcelException{
		basicDumpAll(new File("/home/yama/")
	...
	public void testEventDump1() throws ExcelException{
		eventDumpAll(new File("/home/yama/")
						,BIFFSetting.SST_TYPE_DEFAULT);
	}
